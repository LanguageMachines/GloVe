...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    if ((i = find_arg("-vector-size", argc, argv)) > 0) {
      vector_size = atoi(argv[i + 1]);
    }
    if ((i = find_arg("-iter", argc, argv)) > 0) {
      num_iter = atoi(argv[i + 1]);
    }
    if ((i = find_arg("-threads", argc, argv)) > 0) {
      num_threads = atoi(argv[i + 1]);
    }
    cost = (real*)malloc(sizeof(real) * num_threads);
    if ((i = find_arg("-alpha", argc, argv)) > 0) {
      alpha = atof(argv[i + 1]);
    }
    if ((i = find_arg("-x-max", argc, argv)) > 0) {
      x_max = atof(argv[i + 1]);
    }
    if ((i = find_arg("-eta", argc, argv)) > 0) {
      eta = atof(argv[i + 1]);
    }
    if ((i = find_arg("-binary", argc, argv)) > 0) {
      use_binary = atoi(argv[i + 1]);
    }
    if ((i = find_arg("-model", argc, argv)) > 0) {
      model = atoi(argv[i + 1]);
    }
    if (model != 0 && model != 1) {
      model = 2;
    }
    if ((i = find_arg("-save-gradsq", argc, argv)) > 0) {
    ...
    if ((i = find_arg("-save-file", argc, argv)) > 0) {
      strcpy(save_W_file, argv[i + 1]);
    }
    else {
      strcpy(save_W_file, "vectors");
    }
    if ((i = find_arg( "-gradsq-file", argc, argv)) > 0) {
      strcpy(save_gradsq_file, argv[i + 1]);
      save_gradsq = 1;
    }
    else if (save_gradsq > 0) {
      strcpy(save_gradsq_file, "gradsq");
    }
    if ((i = find_arg("-input-file", argc, argv)) > 0) {
      strcpy(input_file, argv[i + 1]);
    }
    else {
      strcpy(input_file, "cooccurrence.shuf.bin");
    }
    if ((i = find_arg( "-checkpoint-every", argc, argv)) > 0) {
