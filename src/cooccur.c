...
int get_word( char *word, FILE *fin) {
  ...
  while (!feof(fin)) {
    ...
    if (i >= MAX_STRING_LENGTH - 1){
      i--;   // truncate words that exceed max length
    }
  }
  word[i] = 0;
  return 0;
...
int merge_files(int num) {
  ...
  while (size > 0) {
    ...
    if ((counter%100000) == 0 && verbose > 1){
      fprintf(stderr,"\033[39G%lld lines.",counter);
    }
    i = pq[0].id;
    delete_it(pq, size);
    fread(&_new, sizeof(CREC), 1, fid[i]);
...
int get_cooccurrence() {
  ...
  if (verbose > 1) {
    fprintf(stderr,"Processing token: 0");
  }
  ...
  while (1) {
    ...
    if (flag == 1) {  // Newline, reset line index (j)
      j = 0;
      continue;
    }
    counter++;
    if ( (counter%100000) == 0 && verbose > 1) {
      fprintf(stderr,"\033[19G%lld",counter);
    }
	...
	if (symmetric > 0) {
	  bigram_table[lookup[w2-1] + w1 - 2] += 1.0/((real)(j-k)); // If symmetric context is used, exchange roles of w2 and w1 (ie look at right context too)
	}
      }
  ...
  if (verbose > 1) {
    fprintf(stderr,"\033[0GProcessed %lld tokens.\n",counter);
  }
  qsort(cr, ind, sizeof(CREC), compare_crec);
  write_chunk(cr,ind,foverflow);
  ...
  fclose(fid);
  if (verbose > 1) {
    fprintf(stderr,"%d files in total.\n",fidcounter + 1);
  }
  fclose(foverflow);
  free(cr);
  free(lookup);
...
int main(int argc, char **argv) {
  ...
  if ((i = find_arg("-symmetric", argc, argv)) > 0) {
    symmetric = atoi(argv[i + 1]);
  }
  if ((i = find_arg("-window-size", argc, argv)) > 0) {
    window_size = atoi(argv[i + 1]);
  }
  if ((i = find_arg("-vocab-file", argc, argv)) > 0) {
    strcpy(vocab_file, argv[i + 1]);
  }
  else {
    strcpy(vocab_file, "vocab.txt");
  }
  if ((i = find_arg( "-overflow-file", argc, argv)) > 0) {
    strcpy(file_head, argv[i + 1]);
  }
  else {
    strcpy(file_head, "overflow");
  }
  ...
  if ((i = find_arg("-overflow-length", argc, argv)) > 0) {
